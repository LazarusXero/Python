...
def pause_led():
    ...
    while True:
        if GPIO.event_detected(23):
            print("Unpaused")
            return("no")   # if unpaused, the program should continue
        elif GPIO.event_detected(18):
            return("yes")  # if blue button is pressed, the program should stop when returned
        else:              # if nothing is pushed, wait
            continue
...
button_list = [18, 23]                                 # button GPIO channels
led_channel_list = [2, 3, 4, 17, 27, 10, 9, 11, 5, 6]  # led GPIO channels
GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)
GPIO.setup(led_channel_list, GPIO.OUT, initial=GPIO.LOW)   # initial LED setup
GPIO.setup(button_list, GPIO.IN, pull_up_down=GPIO.PUD_UP) # initial button setup
...
dutycycle_index = [2, 25, 100, 25, 2]       # index to change duty cycle on LEDs to be lit up
...
try:
    while True:
        sleep(0.05) # fast sweep
        turn_led_on(center_led, shift_index, dutycycle_index)
        if GPIO.event_detected(18):   # detect blue button to exit
            break
        elif GPIO.event_detected(23): # detect red button to pause
            stop = pause_led()        
            if stop == "yes":         # check to see if blue button was pushed in pause function
                break
            else:                     # if not, just continue with led shifting
                continue
        elif direction == "left":    # shifting left
            print("Shifting left")
